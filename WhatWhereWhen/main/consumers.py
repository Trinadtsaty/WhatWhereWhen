...
class StartConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        await self.clean_stage_if_room_missing(self.room_id)
        # self.stage["captain_id"] = await self.get_your_id(room, "captain")

        if self.room_id not in StartConsumer.stages:
            StartConsumer.stages[self.room_id] = {
                "stage": "collecting",
        ...
        self.stage = StartConsumer.stages[self.room_id]
    ...
    async def room_exists(room_id):
        # Предполагается, что у вас есть модель Room
        from .models import Room

        # Django ORM не поддерживает асинхронных запросов напрямую,
        # поэтому используем sync_to_async
        from asgiref.sync import sync_to_async

        exists = await sync_to_async(Room.objects.filter(id=room_id).exists)()
        return exists

    async def clean_stage_if_room_missing(self, room_id):
        exists = await self.room_exists(room_id)
        if not exists and room_id in StartConsumer.stages:
            del StartConsumer.stages[room_id]